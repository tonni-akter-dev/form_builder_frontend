...
export default function EditForm() {
  ...
  const transformBackendField = (backendField: any): FormField => {
    ...
    if (["radio", "checkbox"].includes(backendField.type)) {
      frontendField.options = backendField.options || ["Option 1", "Option 2"];
    }

    if (backendField.type === "image_text") {
      frontendField.imageUrl = backendField.imageUrl || "";
      frontendField.imagePrompt = backendField.imagePrompt || "";
      frontendField.imagePreview = backendField.imageUrl || "";
    }
      ...
      <div className="space-y-1">
        {fieldTypeOptions.map((option) => (
          <button
            ...
            type="button"
            onClick={() => addField(option.type)}
            className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-lg w-full text-left transition-colors"
          >
            <span className="text-2xl w-8">{option.icon}</span>
            <div className="flex-1">
              ...
              <div className="space-y-4">
                  ...
                  {!field.imagePreview ? (
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
                      ...
                      <label htmlFor={`image-upload-${field.id}`} className="cursor-pointer block">
                        ...
                        <p className="text-xs text-gray-500">PNG, JPG, GIF up to 2MB (Optional)</p>
                      </label>
                    </div>
                  ) : (
                    <div className="relative border rounded-lg p-2 bg-gray-50">
                      <img src={field.imagePreview} alt="Uploaded" className="max-w-full h-auto rounded max-h-64 mx-auto" />
                      ...
                      {field.imageUrl && !uploadingFieldId && (
                        <div className="absolute bottom-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded">Uploaded ✓</div>
                      )}
                      <button
                        type="button"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => updateField(field.id, "imagePrompt", e.target.value)}
                    rows={3}
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 bg-blue-50">
                  ...
                  {field.imagePreview && (
                    <img src={field.imagePreview} alt="Preview" className="max-w-full h-auto rounded mb-3 max-h-48 mx-auto" />
                  )}
                  {field.imagePrompt && <p className="text-sm text-gray-600 mb-3">{field.imagePrompt}</p>}
                  {!field.imagePreview && !field.imagePrompt && (
                    <p className="text-sm text-gray-400 italic">Upload an image and add instructions to see preview</p>
                  )}
                  <textarea disabled rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50" placeholder="Students will write their answer here..." />
                </div>
              </div>
            )}
          </div>
  ...
  const renderPreviewField = (field: FormField, index: number) => {
    return (
      <div key={field.id} className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        <div className="flex items-start gap-3">
          <span className="text-sm font-medium text-gray-400 min-w-6">{index + 1}.</span>
          <div className="flex-1">
            <div className="flex justify-between items-start mb-3">
              <div>
              ...
              <div className="text-sm text-purple-600 font-medium">
                {field.marks} {field.marks === 1 ? 'mark' : 'marks'}
                {requiresManualGrading(field.type) && ' (Manual)'}
              </div>
            </div>

            <div>
              {field.type === "text" && (
                <input type="text" className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#673AB7] focus:border-transparent outline-none" placeholder="Your answer" />
              )}
                ...
                <div className="space-y-2">
                  {field.options?.map((option, idx) => (
                    <label key={idx} className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" name={`preview-${field.id}`} className="w-4 h-4 text-[#673AB7] border-gray-300 focus:ring-[#673AB7]" />
                      <span className="text-sm text-gray-700">{option}</span>
                    </label>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  {field.imagePreview && (
                    <div className="border rounded-lg p-2 bg-gray-50">
                      <img src={field.imagePreview} alt="Question reference" className="max-w-full h-auto rounded max-h-64 mx-auto" />
                    </div>
                  )}
                  {field.imagePrompt && <p className="text-sm text-gray-600">{field.imagePrompt}</p>}
                  <textarea rows={4} className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#673AB7] focus:border-transparent outline-none" placeholder="Your answer based on the image above..." />
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex justify-end mb-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-1 inline-flex">
            <button
              ...
              onClick={() => setIsPreviewMode(false)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                !isPreviewMode ? "bg-[#673AB7] text-white" : "text-gray-600 hover:text-gray-900"
              }`}
            >
              Edit
            </button>
