...
//     return fieldType === "image_text" || fieldType === "file_upload";
//   };
...
//   const uploadFieldFiles = async (fieldId: string, attachments: FileAttachment[]): Promise<string[]> => {
...
//           ...prev,
//           [fieldId]: prev[fieldId].map(f => 
//             f.id === attachment.id ? { ...f, uploadStatus: "success", uploadedUrl: url } : f
//           ),
//         }));
//       } catch (error) {
...
//   const validateField = (field: FormField, value: any, attachments: FileAttachment[]): string => {
//     if (field.required) {
...
//       formDataToSend.append("timeSpent", String(form.duration * 60 - timeRemaining));
...
//       const hasManualGradingFlag = responseData.response?.hasManualGrading || 
...
//         : isAutoSubmit ? "Time's up! Exam submitted automatically!" : "Exam submitted successfully!";
      
//       toast.success(message);
//     } catch (err: any) {
//       console.error("Error submitting form:", err);
//       const errorMessage = err.response?.data?.error || err.message || "Failed to submit form";
...
//                 {field.marks} {field.marks === 1 ? 'mark' : 'marks'} (Manual Grading)
//               </span>
//             )}
//           </div>
//           {field.description && (
...
//               <span className="font-medium">Instruction:</span> {field.imagePrompt}
//             </p>
//           </div>
//         )}
...
//                 <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H8a4 4 0 01-4-4v-8m32 0l-6-6m-8 16l-8-8m8 8l-8-8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
//               </svg>
...
//                     multiple
//                     className="sr-only"
//                     onChange={(e) => handleFileUpload(field._id, e.target.files)}
//                     disabled={examSubmitted}
...
//                   <div key={attachment.id} className="relative border rounded-lg p-2 bg-gray-50">
...
//                       disabled={examSubmitted || attachment.uploadStatus === "uploading"}
//                     >
...
//           </div>
//           {field.marks && (
//             <span className={`text-sm font-medium ${needsManualGrading(field.type) ? 'text-orange-600' : 'text-purple-600'}`}>
...
//                     disabled={examSubmitted}
//                   />
//                   <label htmlFor={`${field._id}-${option}`} className="ml-3 text-sm text-gray-700">
//                     {option}
//                   </label>
//                 </div>
//               ))}
...
//                         key={starValue}
//                         type="button"
//                         onClick={() => handleInputChange(field._id, starValue)}
...
//                           key={numValue}
//                           type="button"
//                           onClick={() => handleInputChange(field._id, numValue)}
...
//                         key={emojiValue}
//                         type="button"
//                         onClick={() => handleInputChange(field._id, emojiValue)}
...
//                     Some questions require manual grading by the teacher. Your score will be updated once the teacher reviews your submissions.
//                   </p>
//                 </div>
//               </div>
//             )}
...
//                 </div>
//                 <div className="text-sm text-gray-600 mt-1">Auto-Graded Score</div>
//               </div>
...
//                 </div>
//                 <div className="text-sm text-gray-600 mt-1">Auto-Graded Marks</div>
//               </div>
//             </div>
...
//                     {!answer.isCorrect && answer.correctAnswer && !answer.needsManualGrading && (
//                       <div>
...
//           </svg>
//           <h3 className="mt-4 text-lg font-medium text-gray-900">Exam Not Published</h3>
//           <p className="mt-2 text-gray-600">This exam is currently in draft mode and not available.</p>
//         </div>
//       </div>
//     );
//   }
...
//           </svg>
//           <h3 className="mt-4 text-lg font-medium text-gray-900">Exam Already Submitted</h3>
//           <p className="mt-2 text-gray-600">You have already submitted this exam. Multiple submissions are not allowed.</p>
//           <button onClick={() => router.push("/")} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Go to Home</button>
//         </div>
//       </div>
//     );
//   }
...
//               <div className="flex justify-between"><span className="text-gray-600">Subject:</span><span className="font-medium text-gray-800">{form.subject}</span></div>
//               <div className="flex justify-between"><span className="text-gray-600">Class:</span><span className="font-medium text-gray-800">{form.className}</span></div>
//               <div className="flex justify-between"><span className="text-gray-600">Duration:</span><span className="font-medium text-orange-600">{form.duration} minutes</span></div>
//               <div className="flex justify-between"><span className="text-gray-600">Total Questions:</span><span className="font-medium text-gray-800">{form.fields.length}</span></div>
//               <div className="flex justify-between"><span className="text-gray-600">Total Marks:</span><span className="font-medium text-purple-600">{form.totalMarks || form.fields.reduce((sum, f) => sum + (f.marks || 1), 0)}</span></div>
//             </div>
//           </div>
...
//               <li>For image-based questions, you can upload multiple files</li>
//               <li>Files will be uploaded to ImgBB cloud storage</li>
//               <li>Manual grading questions will be reviewed by the teacher</li>
//             </ul>
//           </div>

//           <button onClick={startExam} className="w-full bg-orange-500 hover:bg-orange-600 text-white font-medium py-3 rounded-lg transition-colors">
//             Start Exam
//           </button>
//         </div>
//       </div>
...
//                 <div className="h-full bg-orange-500 transition-all duration-1000" style={{ width: `${getTimePercentage()}%` }} />
//               </div>
//             </div>
//           </div>
//         </div>
...
//               <div key={field._id} className="bg-white rounded-lg shadow-sm p-4">
...
//             </button>
//             <p className="text-xs text-gray-500">Make sure to submit before time runs out</p>
//           </div>
//         </form>
//       </div>
//     </div>
        ...
        <div className="max-w-2xl w-full bg-white rounded-lg shadow-sm p-8">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            ...
            <ul className="text-sm text-yellow-700 space-y-1 list-disc list-inside">
              ...
              <li>Make sure you have a stable internet connection</li>
              <li>Do not refresh the page during the exam</li>
              <li>For short answer and image-based questions, you can upload multiple files</li>
              <li>Files will be uploaded to ImgBB cloud storage</li>
              <li>Manual grading questions will be reviewed by the teacher</li>
            </ul>
          </div>
